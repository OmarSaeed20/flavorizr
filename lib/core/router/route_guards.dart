...
/// Guard that ensures user has selected a language before proceeding.
///
/// Redirects to language selection page if `language_selected` is false
/// in SharedPreferences. This guard should be registered BEFORE
/// [OnboardingGuard] in the global guard chain.
class LanguageSelectionGuard extends RouteGuard {
  const LanguageSelectionGuard({required this.isLanguageSelected});

  final Future<bool> Function() isLanguageSelected;

  @override
  Future<bool> canActivate(BuildContext context, GoRouterState state) async {
    final location = state.matchedLocation;

    // Allow access to language selection page itself, splash, and error pages
    if (location == Routes.languageSelection ||
        location == Routes.splash ||
        location == Routes.error ||
