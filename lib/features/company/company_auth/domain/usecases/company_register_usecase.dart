...
class CompanyRegisterUseCase {
  ...
  Future<ApiResult<AuthTokens>> execute(CompanyRegisterParameters parameters) async {
    // Validate company type
    if (parameters.companyType.isEmpty) {
      return ApiResult.failure(const ValidationException(message: 'Company type is required'));
    }

    // Validate company type is "company"
    if (parameters.companyType != 'company') {
      return ApiResult.failure(const ValidationException(message: 'Invalid company type'));
    }

    // Validate name
    if (parameters.name.isEmpty) {
      return ApiResult.failure(const ValidationException(message: 'Company name is required'));
    }
    ...
    if (parameters.phoneIso2Code.isEmpty) {
      return ApiResult.failure(const ValidationException(message: 'Phone ISO2 code is required'));
    }
    ...
    if (parameters.countryId <= 0) {
      return ApiResult.failure(const ValidationException(message: 'Invalid country ID'));
    }

    // Validate governorate ID
    if (parameters.governorateId <= 0) {
      return ApiResult.failure(const ValidationException(message: 'Invalid governorate ID'));
    }

    // Validate birthdate
    if (parameters.birthdate.isEmpty) {
      return ApiResult.failure(const ValidationException(message: 'Birthdate is required'));
    }
    ...
    if (!birthdateRegex.hasMatch(parameters.birthdate)) {
      return ApiResult.failure(
        const ValidationException(message: 'Invalid birthdate format. Use YYYY-MM-DD'),
      );
    }

    // Validate gender
    if (parameters.gender.isEmpty) {
      return ApiResult.failure(const ValidationException(message: 'Gender is required'));
    }

    // Validate gender value
    if (parameters.gender != 'male' && parameters.gender != 'female') {
      return ApiResult.failure(const ValidationException(message: 'Invalid gender value'));
    }
